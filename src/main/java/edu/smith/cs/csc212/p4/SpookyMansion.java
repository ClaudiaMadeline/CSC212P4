...
public class SpookyMansion implements GameWorld {
	...
	public SpookyMansion() {
		...
		basement.addExit(new Exit("cellar", "There is a small archway."));
		basement.addExit(new SecretExit("secretRoom", "There is a creepy door."));
		
		Place cellar = insert(Place.create("cellar", "You have found a cellar housing rats."));
		cellar.addExit(new Exit("basement", "Go back to the basement."));
		...
		attic2.addExit(new Exit ("balcony", "There is a glass door with light streaming through."));
		
		Place balcony = insert(Place.create("balcony", 
				"You step onto a balcony with a view of the front yard."));
		balcony.addExit(new Exit("attic2", "There is more back through the glass door."));
		...
		secretRoom.addExit(new Exit("basement", "Go to the basement."));
		...
		for (int i=0; i<hallwayDepth; i++) {
			Place hallwayPart = insert(Place.create("hallway"+i, "This is a very long hallway.\n"
																+ "You see a " + (i+1) + " scratched on the wall."));
			if (i == 0) {
